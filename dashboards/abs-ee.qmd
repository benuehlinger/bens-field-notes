...
{
  const latest = (delinqData && !delinqData.error && delinqData.length > 0)
    ? delinqData[delinqData.length - 1] : null

  const rate = latest
    ? `${Number(latest.delinq_rate_90plus).toFixed(2)}%`
    : "—"
  const loans = latest
    ? (latest.total_loans >= 1e6
        ? `${(latest.total_loans / 1e6).toFixed(1)}M`
  ...
  return html`<div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(170px,1fr));gap:1rem;margin:0 0 2rem;">
    <div class="stat-box"><div class="stat-value">${rate}</div><div class="stat-label">90+ Day Delinquency</div></div>
    <div class="stat-box"><div class="stat-value">${loans}</div><div class="stat-label">Loans in Latest Month</div></div>
  </div>`
}
```
...

{
  ...
  return Plot.plot({
    ...
    caption: "Source: SEC EDGAR ABS-EE | Units: loan count | As of latest reporting period"
  })
}
```
